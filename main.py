...
def display_cards(facts_list, key_offset=0):
    if not facts_list:
        st.info("No facts found matching the selected criteria.")
        return
    
    # Sort by date
    sorted_facts = sorted(facts_list, key=lambda x: x['date'].split('-')[0])
        ...
        with st.expander(f"**{fact['date']}** - {fact.get('doc_name', 'Unknown Document')}", expanded=False):
            ...
            if evidence_content:
                ...
                for evidence in evidence_content:
                    ...
                    with col1:
                        if st.button(f"Preview {evidence['id']}", key=f"preview_{evidence['id']}_{i}_{key_offset}"):
...
# Function to display document categories
def display_document_categories(facts_data):
    document_sets = get_document_sets()
    
    for docset in document_sets:
        if docset.get('isGroup'):
            category_facts = []
            
            # Simple assignment based on document name matching
            for fact in facts_data:
                doc_name = fact.get('doc_name', '').lower()
                category_name = docset['name'].lower()
                
                # Match facts to categories
                if (category_name in doc_name or 
                    any(doc['name'].lower() in doc_name for doc in docset.get('documents', []))):
                    category_facts.append(fact)
            
            # Default assignments for demo
            if not category_facts:
                if docset['id'] == 'appeal':
                    category_facts = [f for f in facts_data if 'appeal' in f.get('doc_name', '').lower()]
                elif docset['id'] == 'provisional_measures':
                    category_facts = [f for f in facts_data if 'provisional' in f.get('doc_name', '').lower()]
            
            # Display category
            with st.expander(f"📁 {docset['name']} ({len(category_facts)} facts)", expanded=False):
                if category_facts:
                    for fact in category_facts:
                        st.write(f"**{fact['date']}** - {fact['event']}")
                        ...
                        st.write("**Party Positions:**")
                        
                        claimant_text = fact.get('claimant_submission', 'No submission recorded')
                        if claimant_text != 'No specific submission recorded':
                            st.info(f"**Claimant:** {claimant_text[:200]}...")
                        
                        respondent_text = fact.get('respondent_submission', 'No submission recorded')
                        if respondent_text != 'No specific submission recorded':
                            st.warning(f"**Respondent:** {respondent_text[:200]}...")
                        
                        st.write("---")
                else:
                    st.info("No facts found in this category")
...
def main():
    ...
    if st.session_state.view == "Facts":
        # Simple HTML component for navigation styling (minimal height)
        html_content = """
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <!-- Minimal placeholder for styling -->
        </body>
        </html>
        """
        
        # Render minimal HTML component
        ...
        # Main view selection
        view_tab1, view_tab2 = st.tabs(["Card View", "Document Categories"])
        
        with view_tab1:
            # Card view with filtering tabs
            filter_tab1, filter_tab2, filter_tab3 = st.tabs(["All Facts", "Disputed Facts", "Undisputed Facts"])
            
            with filter_tab1:
                display_cards(facts_data, 0)
            
            with filter_tab2:
                disputed_facts = [fact for fact in facts_data if fact['isDisputed']]
                display_cards(disputed_facts, 100)
                
            with filter_tab3:
                undisputed_facts = [fact for fact in facts_data if not fact['isDisputed']]
                display_cards(undisputed_facts, 200)
        
        with view_tab2:
            display_document_categories(facts_data)
