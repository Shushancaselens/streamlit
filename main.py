                            ...
                            if st.button("📋", key=copy_key, help="Copy Reference", use_container_width=True):
                                ...
                                if (navigator.clipboard && window.isSecureContext) {{
                                    navigator.clipboard.writeText(textToCopy).then(function() {{
                                        console.log('Reference copied to clipboard successfully');
                                    }}).catch(function(err) {{
                                    ...
                                    try {{
                                        ...
                                        console.log('Reference copied using fallback method');
                                    }} catch (err) {{
...
def main():
    ...
    if st.session_state.view == "Facts":
        # Add global CSS and JavaScript for clipboard functionality
        st.markdown("""
        <style>
        ...
        with col_copy:
            if st.button("📋 Copy", use_container_width=True, type="secondary", key="header_copy_btn"):
                ...
                copy_text = "CASE FACTS SUMMARY\n"
                copy_text += "=" * 50 + "\n\n"
                
                for fact in all_facts:
                    copy_text += f"Date: {fact['date']}\n"
                    copy_text += f"Event: {fact['event']}\n"
                    copy_text += f"Status: {'Disputed' if fact['isDisputed'] else 'Undisputed'}\n"
                    copy_text += f"Exhibits: {', '.join(fact.get('exhibits', []))}\n"
                    copy_text += f"Document: {fact.get('doc_name', 'N/A')}\n"
                    if fact.get('page'):
                        copy_text += f"Page: {fact['page']}\n"
                    copy_text += f"Source: {fact.get('source_text', 'N/A')}\n"
                    copy_text += "-" * 30 + "\n\n"
                
                # Use JavaScript to copy to clipboard
                components.html(f"""
                <script>
                const textToCopy = `{copy_text.replace('`', '\\`').replace('
...
if __name__ == "__main__":
    main())}`;
                ...
                if (navigator.clipboard && window.isSecureContext) {{
                    navigator.clipboard.writeText(textToCopy).then(function() {{
                        console.log('Facts copied to clipboard successfully');
                    }}).catch(function(err) {{
                        console.error('Failed to copy facts: ', err);
                    }});
                }} else {{
                    const textArea = document.createElement('textarea');
                    textArea.value = textToCopy;
                    ...
                    try {{
                        document.execCommand('copy');
                        console.log('Facts copied using fallback method');
                    }} catch (err) {{
                        console.error('Fallback copy failed: ', err);
                    }}
                    document.body.removeChild(textArea);
        ...
        with col_export:
            if st.button("📥 Export", use_container_width=True, type="secondary", key="header_export_btn"):
                ...
                export_content = "LEGAL CASE FACTS - DETAILED EXPORT\n"
                export_content += "Generated: " + pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S") + "\n"
                export_content += "=" * 60 + "\n\n"
                ...
                export_content += f"SUMMARY STATISTICS:\n"
                export_content += f"Total Facts: {total_facts}\n"
                export_content += f"Disputed Facts: {disputed_facts}\n"
                export_content += f"Undisputed Facts: {undisputed_facts}\n\n"
                export_content += "=" * 60 + "\n\n"
                
                # Add detailed facts
                for i, fact in enumerate(all_facts, 1):
                    export_content += f"FACT #{i}\n"
                    export_content += f"Date: {fact['date']}\n"
                    export_content += f"Event: {fact['event']}\n"
                    export_content += f"Status: {'DISPUTED' if fact['isDisputed'] else 'UNDISPUTED'}\n"
                    export_content += f"Parties Involved: {', '.join(fact.get('parties_involved', []))}\n"
                    export_content += f"Exhibits: {', '.join(fact.get('exhibits', []))}\n"
                    export_content += f"Document: {fact.get('doc_name', 'N/A')}\n"
                    export_content += f"Page: {fact.get('page', 'N/A')}\n"
                    export_content += f"Paragraphs: {fact.get('paragraphs', 'N/A')}\n"
                    export_content += f"Document Summary: {fact.get('doc_summary', 'N/A')}\n\n"
                    
                    export_content += f"SOURCE TEXT:\n{fact.get('source_text', 'N/A')}\n\n"
                    
                    export_content += f"CLAIMANT SUBMISSION:\n{fact.get('claimant_submission', 'No submission recorded')}\n\n"
                    
                    export_content += f"RESPONDENT SUBMISSION:\n{fact.get('respondent_submission', 'No submission recorded')}\n\n"
                ...
                components.html(f"""
                <script>
                const content = `{export_content.replace('`', '\\`').replace('
