...
# Function to render fact cards
def render_fact_cards(filtered_facts, tab_suffix=""):
...
def main():
        ...
        <div style="display: flex; align-items: center; margin-bottom: 20px;">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 175 175" width="35" height="35">
              ...
              <g mask="url(#whatsapp-mask)">
                ...
                <path fill-rule="evenodd" clip-rule="evenodd" d="M137367 54.0014C126.648 40.3105 110.721 32.5723 93.3045 32.5723C63.2347 32.5723 38.5239 57.1264 38.5239 87.0377C38.5239 96.9229 41.1859 106.155 45.837 114.103L45.6925 113.966L37.918 141.957L65.5411 133.731C73.8428 138.579 83.5458 141.355 93.8997 141.355C111.614 141.355 127.691 132.723 137.664 119.628L114.294 101.621C109.53 108.467 101.789 112.187 93.4531 112.187C79.4603 112.187 67.9982 100.877 67.9982 87.0377C67.9982 72.9005 79.6093 61.7396 93.751 61.7396C102.236 61.7396 109.679 65.9064 114.294 72.3052L137.367 54.0014Z" fill="white"/>
              </g>
            </svg>
    ...
    # Facts view with clean tabs and natural spacing
    if st.session_state.view == "Facts":
        # Add natural spacing using markdown
        st.markdown("")
        ...
        with tab1:
            ...
            render_fact_cards(filtered_facts, "all")
        
        with tab2:
            filtered_facts = [fact for fact in facts_data if fact['isDisputed']]
            filtered_facts.sort(key=lambda x: x['date'].split('-')[0])
            render_fact_cards(filtered_facts, "disputed")
        
        with tab3:
            filtered_facts = [fact for fact in facts_data if not fact['isDisputed']]
            filtered_facts.sort(key=lambda x: x['date'].split('-')[0])
            render_fact_cards(filtered_facts, "undisputed")
    
    elif st.session_state.view == "Arguments":
        st.header("Arguments View")
        st.info("Arguments functionality would be implemented here")
    
    elif st.session_state.view == "Exhibits":
        st.header("Exhibits View")
        st.info("Exhibits functionality would be implemented here")
