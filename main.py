// Add function to align sections between appellant and respondent
            function alignSections() {
                // Get all rows
                const rows = document.querySelectorAll('tr.argument-row');
                ...
                rows.forEach(row => {
                    ...
                    // Find all section wrappers in each cell
                    const appellantSections = cells[0].querySelectorAll('.section-wrapper');
                    const respondentSections = cells[1].querySelectorAll('.section-wrapper');
                    
                    // Create a map of section types to align
                    const sectionMap = new Map();
                    
                    // Add appellant sections to map
                    appellantSections.forEach(section => {
                        const type = section.getAttribute('data-section');
                        if (!sectionMap.has(type)) {
                            sectionMap.set(type, { appellant: section, respondent: null });
                        } else {
                            sectionMap.get(type).appellant = section;
                        }
                    });
                    
                    // Add respondent sections to map
                    respondentSections.forEach(section => {
                        const type = section.getAttribute('data-section');
                        if (!sectionMap.has(type)) {
                            sectionMap.set(type, { appellant: null, respondent: section });
                        } else {
                            sectionMap.get(type).respondent = section;
                        }
                    });
                    
                    // Apply min-heights to align sections
                    sectionMap.forEach((pair, type) => {
                        if (pair.appellant && pair.respondent) {
                            const appellantHeight = pair.appellant.offsetHeight;
                            const respondentHeight = pair.respondent.offsetHeight;
                            const maxHeight = Math.max(appellantHeight, respondentHeight);
                            
                            pair.appellant.style.minHeight = `${maxHeight}px`;
                            pair.respondent.style.minHeight = `${maxHeight}px`;
                        }
                    });
                });
