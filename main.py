...
def get_argument_data():
    # Data structure remains the same as before
    claimant_args = {
        "1": {
            "id": "1",
...
def main():
    ...
    <head>
        <style>
            ...
            /* Argument card and details - with alignment fixes */
            .argument {{
            ...
            .argument-header {{
                ...
                align-items: center;
                height: 24px; /* Fixed height for headers */
            }}
            ...
            /* Child arguments container - with alignment improvements */
            .argument-children {{
                padding-left: 1.5rem;
                display: none;
                position: relative;
            ...
            /* Child arguments synchronized grid layout */
            .paired-children-container {{
                display: grid;
                grid-template-columns: 1fr 1fr;
                gap: 1.5rem;
                width: 100%;
            ...
            /* Sections for content need equal height */
            .section-pair {{
                display: flex;
                width: 100%;
            }}
            ...
            /* Content sections need equal height */
            .content-section {{
                margin-bottom: 1.5rem;
            }}
            ...
            /* Fixed height components for alignment */
            .legal-point, .factual-point {{
                margin-bottom: 0.5rem;
                min-height: 80px;
                display: flex;
                flex-direction: column;
            ...
            /* Overview points with fixed height */
            .overview-block {{
                background-color: #f7fafc;
                border-radius: 0.5rem;
                padding: 1rem;
    ...
    <body>
        ...
        <script>
            ...
            // Render a pair of arguments (claimant and respondent) - with improved alignment
            ...
            // Render the topic view - with improved alignment
            ...
            // Toggle argument expansion - updated for synchronization and equal heights
            function toggleArgument(fullId, argPath) {{
                ...
                // After toggle, ensure both sides have the same height
                if (!isExpanded) {{
                    setTimeout(() => {{
                        syncContentHeights(fullId, pairedId);
                    }}, 50);
                }}
            }}
            
            // Synchronize heights of paired elements
            function syncContentHeights(id1, id2) {{
                const el1 = document.getElementById(`content-${{id1}}`);
                const el2 = document.getElementById(`content-${{id2}}`);
                
                if (el1 && el2) {{
                    // Reset heights to auto first to calculate natural heights
                    ...
                    // Set both to the tallest height
                    el1.style.height = `${{maxHeight}}px`;
                    el2.style.height = `${{maxHeight}}px`;
                    
                    // Also sync any child section heights
                    const sections1 = el1.querySelectorAll('.content-section');
                    const sections2 = el2.querySelectorAll('.content-section');
                    
                    if (sections1.length === sections2.length) {{
                        for (let i = 0; i < sections1.length; i++) {{
                            const points1 = sections1[i].querySelectorAll('.legal-point, .factual-point, .reference-block');
                            const points2 = sections2[i].querySelectorAll('.legal-point, .factual-point, .reference-block');
                            
                            if (points1.length === points2.length) {{
                                for (let j = 0; j < points1.length; j++) {{
                                    const pointHeight = Math.max(points1[j].scrollHeight, points2[j].scrollHeight);
                                    points1[j].style.height = `${{pointHeight}}px`;
                                    points2[j].style.height = `${{pointHeight}}px`;
                                }}
                            }}
                        }}
                    }}
