        ...
        <body>
            ...
            <script>
                ...
                function renderDocumentPreview() {{
                    const container = document.getElementById('documents-preview-container');
                    container.innerHTML = '';
                    
                    // Create header
                    const headerEl = document.createElement('div');
                    headerEl.style.cssText = 'margin-bottom: 24px; text-align: center;';
                    headerEl.innerHTML = `
                        <h2 style="color: #2d3748; margin-bottom: 8px;">Document Preview</h2>
                        <p style="color: #718096; font-size: 14px;">Preview and analyze all case documents</p>
                    `;
                    container.appendChild(headerEl);
                    
                    // Create document sections based on document sets
                    documentSets.forEach(docset => {{
                        if (docset.isGroup) {{
                            // Create document set section
                            const sectionEl = document.createElement('div');
                            sectionEl.style.cssText = 'margin-bottom: 32px; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;';
                            
                            // Section header
                            const sectionHeaderEl = document.createElement('div');
                            sectionHeaderEl.style.cssText = 'padding: 16px 20px; background-color: #f8fafc; border-bottom: 1px solid #e2e8f0;';
                            sectionHeaderEl.innerHTML = `
                                <div style="display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center;">
                                        <svg class="folder-icon" style="margin-right: 12px;" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path>
                                        </svg>
                                        <h3 style="margin: 0; color: #2d3748; font-size: 18px;">${{docset.name}}</h3>
                                    </div>
                                    <span class="badge ${{docset.party === 'Appellant' ? 'appellant-badge' : (docset.party === 'Respondent' ? 'respondent-badge' : 'shared-badge')}}">
                                        ${{docset.party}}
                                    </span>
                            ...
                            // Documents grid
                            const documentsGridEl = document.createElement('div');
                            documentsGridEl.style.cssText = 'padding: 20px; display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 16px;';
                            
                            docset.documents.forEach(doc => {{
                                // Get facts related to this document
                                const relatedFacts = factsData.filter(fact => 
                                    fact.source && fact.source.includes(doc.id + '.') ||
                                    (fact.doc_name && fact.doc_name.toLowerCase().includes(doc.name.toLowerCase()))
                                );
                                
                                const docCardEl = document.createElement('div');
                                docCardEl.style.cssText = 'border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background-color: white; transition: all 0.2s ease; cursor: pointer;';
                                docCardEl.onmouseover = () => docCardEl.style.boxShadow = '0 4px 12px rgba(0,0,0,0.1)';
                                docCardEl.onmouseout = () => docCardEl.style.boxShadow = 'none';
                                
                                docCardEl.innerHTML = `
                                    <div style="padding: 16px; border-bottom: 1px solid #e2e8f0; background-color: #f8fafc;">
                                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
                                            <h4 style="margin: 0; color: #2d3748; font-size: 14px; font-weight: 600;">${{doc.name}}</h4>
                                            <span style="font-size: 11px; color: #718096; font-weight: 500;">${{doc.id}}</span>
                                        </div>
                                        <div style="display: flex; align-items: center; gap: 8px;">
                                            <span class="badge ${{doc.party === 'Appellant' ? 'appellant-badge' : (doc.party === 'Respondent' ? 'respondent-badge' : 'shared-badge')}}" style="font-size: 10px;">
                                                ${{doc.party}}
                                            </span>
                                            <span class="badge" style="font-size: 10px; background-color: #e2e8f0; color: #4a5568;">
                                                ${{relatedFacts.length}} facts
                                            </span>
                                        </div>
                                    </div>
                                    <div style="padding: 16px;">
                                        <div style="font-size: 12px; color: #718096; line-height: 1.5; margin-bottom: 12px;">
                                            ${{doc.summary || 'Document summary not available'}}
                                        </div>
                                        ${{relatedFacts.length > 0 ? `
                                            <div style="font-size: 11px; color: #4a5568;">
                                                <strong>Key Facts Referenced:</strong>
                                                <div style="margin-top: 6px;">
                                                    ${{relatedFacts.slice(0, 3).map(fact => `
                                                        <div style="margin-bottom: 4px; padding: 4px 8px; background-color: #f8fafc; border-left: 2px solid #cbd5e0; border-radius: 0 3px 3px 0;">
                                                            <span style="font-weight: 500;">${{fact.date}}:</span> ${{fact.event.substring(0, 60)}}${{fact.event.length > 60 ? '...' : ''}}
                                                        </div>
                                                    `).join('')}}
                                                    ${{relatedFacts.length > 3 ? `<div style="font-size: 10px; color: #718096; margin-top: 4px;">... and ${{relatedFacts.length - 3}} more</div>` : ''}}
                                                </div>
                                            </div>
                                        ` : '<div style="font-size: 11px; color: #a0aec0; font-style: italic;">No facts currently linked to this document</div>'}}
                                    </div>
                                `;
                                
                                documentsGridEl.appendChild(docCardEl);
                            }});
                            
                            sectionEl.appendChild(documentsGridEl);
                            container.appendChild(sectionEl);
                        }}
                    }});
                    
                    // If no document sets found
                    if (documentSets.filter(ds => ds.isGroup).length === 0) {{
                        container.innerHTML = '<p style="text-align: center; padding: 40px; color: #718096;">No documents found.</p>';
                    }}
                }}
